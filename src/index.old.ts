...
type GetCaptionsArgs = z.infer<typeof GetCaptionsArgsSchema>;
type CaptionEntry = z.infer<typeof CaptionEntrySchema>;
...
class YouTubeCaptionsMCPServer {
  ...
  private async getCaptions(args: any) {
    ...
    try {
      ...
      const formattedCaptions = validatedCaptions
        .map((caption: CaptionEntry, index: number) => {
          ...
          return `[${
            index + 1
          }] ${startTime} - ${endTime}: ${caption.text.trim()}`;
        })
        .join("\n");
      ...
      const summary = {
        ...
        totalDuration: this.formatTime(
          Math.max(
            ...validatedCaptions.map(
              (c) => parseFloat(c.start) + parseFloat(c.dur)
            )
          )
        ),
      };
...
${formattedCaptions}`,
  ...
  private async extractVideoId(args: any) {
